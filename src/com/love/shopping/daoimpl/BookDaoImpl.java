...
public class BookDaoImpl implements BookDao {
	...
	public Page doPage(int type,int currentPage,int pageSize){
		...
		List<Book> pageList = new ArrayList<Book>();//每一页的书籍 数据集合
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		DbUtil dbUtil = null;
		String sql = null;
	 ...
	 * @return List
	 */
	public List<Book> searchBooksByConditions(int superTypeId,int subTypeId,String searchMethod,String upLoadTime){
		...
		if(str[0].equals("price")){//选择了按价格查询方式
			if(str[1].equals("大于200")){
				...
				if(upLoadTime.equals("大于30")){
					...
					System.out.println("upLoadDate"+upLoadDate);
					//价格大于200，时间大于一个月
					sql = "select * from tb_book where superTypeId=? and subTypeId=? and "+str[0]+" >?"+" limit "+currentPage+","+pageSize;
					System.out.println("价格大于200，时间大于30天"+sql);
				}else {
					//价格大于200，时间没有一个月
					dateLow = Integer.parseInt(upLoadTime);
					upLoadDate = this.getDate(dateLow);
					...
					System.out.println("nowDate="+nowDate);
					System.out.println("upLoadDate"+upLoadDate);
					sql = "select * from tb_book where superTypeId=? and subTypeId=? and "+str[0]+" >? and inTime between ? and ?"+" limit "+currentPage+","+pageSize;
					System.out.println("价格大于200，时间小于30天"+sql);
				}
			}else{
				priceLower = Float.parseFloat(str[1].split("-")[0]);
				priceTop = Float.parseFloat(str[1].split("-")[1]);
				if(upLoadTime.equals("大于30")){
					...
					nowDate = this.getNowDate();
					System.out.println("nowDate="+nowDate);
					sql = "select * from tb_book where superTypeId=? and subTypeId=? and "+str[0]+" between ? and ?"+" limit "+currentPage+","+pageSize;
					System.out.println("价格小于200，时间大于30"+sql);
				}else {
					//价格小于200，时间没有一个月
					dateLow = Integer.parseInt(upLoadTime);
					upLoadDate = this.getDate(dateLow);
					...
					System.out.println("nowDate="+nowDate);
					System.out.println("upLoadDate"+upLoadDate);
					sql = "select * from tb_book where superTypeId=? and subTypeId=? and "+str[0]+" between ? and ? and inTime between ? and ?"+" limit "+currentPage+","+pageSize;
					System.out.println("价格小于200，时间小于30"+sql);
				}
			}	
		}else {//选择的是其它查询方式
			if(upLoadTime.equals("大于30")){
				...
				//时间大于一个月
				sql = "select * from tb_book where superTypeId=? and subTypeId=? and "+str[0]+"=?"+" limit "+currentPage+","+pageSize;
				System.out.println("其他方式搜索，时间大于30"+sql);
			}else {
				//时间没有一个月
				dateLow = Integer.parseInt(upLoadTime);
				upLoadDate = this.getDate(dateLow);
				...
				System.out.println("upLoadDate"+upLoadDate);
				sql = "select * from tb_book where superTypeId=? and subTypeId=? and "+str[0]+"=? and inTime between ? and ?"+" limit "+currentPage+","+pageSize;
				System.out.println("其他方式搜索，时间小于30"+sql);
			}
		}
		System.out.println("sql="+sql);
		try {
